...
export default function Inbox(props) {
  ...
  useEffect(() => {
    async function atualizaInbox() {
      ...
      const retorno = await api.get(`tickets/gestao/inbox/${id}`);

      const { error = '' } = retorno.data.tickets;

      if (error !== '') {
        toast.warn('Você não tem acesso ao inbox deste usuário.');
  ...
  async function CarregarTicket(id) {
    const response = await api.get(`tickets/gestao/received/${id}`);
    ...
    <>
      <Container>
        <div className="title-header">
          <p>
            Inbox de <strong>{usuario.nome}</strong>
          </p>
        </div>
