...
function Concluidos() {
  ...
  async function CarregaTickets() {
    const response = await api.get('tickets/concluidos');
  ...
  async function funcaoAtualizaTicket(id) {
    ...
    const { data } = await api.get(`tickets/concluidos/${id}`);
    if (data.success) {
      const { ticket } = data;
      setTicketSelecionado(ticket);
      setTickets(
    ...
    <Container>
      <Sidebar>
        <div className="ticket-title">
          <h2>Concluídos</h2>
          <span>(Tíckets enviados por você e que foram concluídos)</span>
        </div>
        <div className="busca">
          <MdSearch />
          <input
                ...
                >
                  <TicketMini
                    ...
                    avaliacao={{
                      mostrar: true,
                      valor:
                        ticketMap.avaliacao === null
                          ? 0
                          : ticketMap.avaliacao.nota,
                    }}
                    conclusao={
