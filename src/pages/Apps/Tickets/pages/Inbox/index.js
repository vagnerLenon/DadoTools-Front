...
function Inbox({ idTicket }) {
  ...
  async function CarregaTickets() {
    ...
    if (ticketDoId.length > 0) {
      ...
      if (idTicket > 0) {
        toast.warn(
          `O ticket de id ${idTicket} não foi encontrado no Inbox. Verifique se ele não está concluído.`
        );
      }
    }
  ...
  async function funcaoAtualizaTicket(id) {
    ...
    const { data } = await api.get(`tickets/inbox/${id}`);
    if (data.success) {
      const { ticket } = data;
      setTicketSelecionado(ticket);
      setTickets(
    ...
    <Container>
      <Sidebar>
        <div className="ticket-title">
          <h2>Inbox</h2>
          <span>
            (Caixa de entrada. Suas demandas que ainda estão pendentes)
          </span>
        </div>
        <div className="busca">
          <MdSearch />
