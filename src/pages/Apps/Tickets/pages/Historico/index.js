...
function Historico() {
  ...
  async function CarregaTickets() {
    const response = await api.get('tickets/historico');
  ...
  function removeNotificacao(id_ticket) {
    api.post('tickets/historico', { id_ticket });
    setNotifications(
  ...
  async function funcaoAtualizaTicket(id) {
    ...
    const { data } = await api.get(`tickets/historico/${id}`);
    if (data.success) {
      const { ticket } = data;
      setTicketSelecionado(ticket);
      setTickets(
    ...
    <Container>
      <Sidebar>
        <div className="ticket-title">
          <h2>Histórico</h2>
          <span>(Tíckets enviados PARA você e que você concluiu)</span>
        </div>
        <div className="busca">
          <MdSearch />
          <input
