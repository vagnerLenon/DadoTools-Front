...
import Encaminhados from './pages/Encaminhados';
...
export default function Tickets(props) {
  ...
  const [encaminhados, setEncaminhados] = useState(false);
  ...
  function renderSwitch() {
    ...
    if (enviados) {
      return <Enviados idTicket={Number(id)} />;
    }
    if (concluidos) {
      return <Concluidos />;
    }
      ...
      <Menus>
        <ul>
          ...
          <MenuItem
            name="encaminhados"
            ativo={encaminhados}
            onClick={() => handleSelectMenu('encaminhados')}
            title="Tickets Que já foram encaminhados para você"
          >
            <MdForward size={15} />
            <p>Encaminhados</p>
          </MenuItem>
